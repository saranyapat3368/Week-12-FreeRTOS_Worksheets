...
|-------|--------|------|---------|-------|-------|---------|
| 1 (ปกติ) | 86 | | 55 | 118 | 30 | 289 | เหตุการณ์มากระจายสมดุล Processor รับทีละเหตุการณ์ผ่าน Queue Set ไม่มี backlog |
| 2 (ไม่มี Sensor) | 0 | 56 | 121 | 30 | 207 | ปริมาณรวมลดลง แต่การตอบสนองเร็วขึ้นเล็กน้อย (โหลดรวมลดลง) |
| 3 (Network เร็ว) | 84 | 52 | 520 | 30 |686 | Network ครองสัดส่วนเหตุการณ์สูงสุด เห็น burst จาก network บ้างแต่ไม่เกิดค้างคาเพราะ Processor ทำได้ ~5 event/วินาที |

### คำถามสำหรับการทดลอง
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
- ใช้ xQueueSelectFromSet(xQueueSet, timeout) ซึ่งคืนค่าเป็น handle ของสมาชิกที่ “พร้อม” (queue/semaphore ที่ active) จากนั้นเปรียบเทียบกับ xSensorQueue, xUserQueue, xNetworkQueue, xTimerSemaphore แล้วค่อย xQueueReceive()/xSemaphoreTake() จาก แหล่งที่ตรงกัน จึงรู้ที่มาได้ทันที
2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
- ไล่เช็คทุกสมาชิก ด้วย timeout = 0 เพื่อ ดึงและประมวลผลให้หมดเป็นรอบๆ หรือกำหนด “ลำดับความสำคัญ” โดยเรียงเช็ค queue ตาม priority ที่ต้องการ
3. Queue Sets ช่วยประหยัด CPU อย่างไร?
- แทนที่จะต้อง polling หลาย queue/semaphore ทีละครั้ง Task เดียวสามารถ บล็อกคอยบน set เดียว ได้ → ลดการตื่นปลุก/สลับ context ที่ไม่จำเป็น, ลดโค้ดวนรอ, และ ตอบสนองทันทีเมื่อมีเหตุการณ์ใดๆ เข้ามา (event-driven แท้)
