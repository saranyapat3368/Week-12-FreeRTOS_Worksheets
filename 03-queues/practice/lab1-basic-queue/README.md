...
|-------|-------------|---------------|--------------|---------|
| 1 | 2s | 1.5s | Normal | Queue มีการขึ้นลงเป็นช่วง ๆ แต่ไม่เต็มหรือว่างนาน|
| 2 | 0.5s | 1.5s | Full | Sender ส่งเร็วกว่า Receiver ทำให้ Queue เต็มในไม่กี่รอบ |
| 3 | 2s | 0.1s | Empty | Receiver รอคิวบ่อยครั้งเพราะไม่มีข้อมูลพอ |

### คำถามสำหรับการทดลอง
1. เมื่อ Queue เต็ม การเรียก `xQueueSend` จะเกิดอะไรขึ้น?
- sender จะ block รอจนกว่าจะมีที่ว่างใน Queue
2. เมื่อ Queue ว่าง การเรียก `xQueueReceive` จะเกิดอะไรขึ้น?
- จะ block รอ จนมีข้อมูลเข้ามาหรือจน timeout
3. ทำไม LED จึงกะพริบตามการส่งและรับข้อความ?
- sender_task ทุกครั้งที่ส่งข้อความสำเร็จจะสั่งให้ LED_SENDER (GPIO2) กระพริบ
- receiver_task ทุกครั้งที่รับข้อความสำเร็จจะสั่งให้ LED_RECEIVER (GPIO4) กระพริบ
