...
CMakeLists.txt  project()  main/CMakeLists.txt main/hello_esp32.c
2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
- .bin = ไฟล์เฟิร์มแวร์พร้อมแฟลชลงบอร์ด (ไม่มีสัญลักษณ์ดีบัก)
- .elf = ไฟล์ที่มีสัญลักษณ์/ดีบักอินโฟ ใช้กับ GDB, ดูขนาด/แม็พหน่วยความจำ แต่ไม่แฟลชลงชิปโดยตรง
3. คำสั่ง `idf.py set-target` ทำอะไร?
- ตั้งค่าเป้าหมายชิป (เช่น esp32, esp32s3), เลือกทูลเชน/ไลบรารีที่ถูกต้อง
- อัปเดต/สร้าง sdkconfig และคอนฟิกบิลด์ให้เข้ากับชิปนั้น
4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
- ootloader/, partition_table/,ฟลเดอร์คอมโพเนนต์ (esp-idf/...) 
- ไฟล์ผลลัพธ์: .bin, .elf, .map, flash_args, compile_commands.json และออบเจ็กต์ .o ต่าง ๆ
5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
- vTaskDelay() ให้ FreeRTOS ปล่อย CPU ให้ทาสก์อื่นทำงาน (ไม่บล็อกทั้งระบบ) และประหยัดพลังงาน
- delay() (สไตล์ Arduino) มักเป็นการหน่วงแบบ busy-wait ทำให้ CPU ว่างงานแต่ยังจับ CPU ไว้ → อาจกระทบทาสก์อื่น/เวลาจริง
