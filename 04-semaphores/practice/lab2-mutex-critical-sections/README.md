...
|-------|------------|--------|-----------|-------------|---------|
| 1 (With Mutex) | 128 | 4 | 0 | 96.9% | CS ถูกป้องกันครบ ไม่มี data corruption; มีบางรอบรอ mutex เกิน 5s |
| 2 (No Mutex) | 0 | 0 | 9 | | 0% | ปิด xSemaphoreTake/Give → ไม่มีตัวนับ Successful/Failed ขยับ แต่เกิด race ชัด (checksum เพี้ยน/ค่าทับกัน) |
| 3 (Changed Priority) | 115 | 12 | 0 | 90.6% | สลับ priority ให้ Low สูงกว่า → แย่ง mutex บ่อยขึ้น มีรอเกินเวลาเพิ่ม แต่ ไม่มี corruption เพราะยังใช้ mutex และ PI ช่วย |

### คำถามสำหรับการทดลอง
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่?
- เกิดได้ง่ายมากครับ/ค่ะ เพราะหลาย task เข้าถึง/แก้ไข shared_data.counter และ shared_buffer พร้อมกันโดยไม่มี mutual exclusion → เกิด race condition เช่น ค่าถูกเขียนทับกัน, counter ไม่ต่อเนื่อง, checksum ไม่ตรงกับข้อมูล
2. Priority Inheritance ทำงานอย่างไร?
- ถ้า Low-priority ถือ mutex อยู่ แล้ว High-priority มาขอและต้องรอ → kernel จะ ยกระดับชั่วคราว (inherit) ของ task ที่ถือ mutex ให้สูงขึ้นเท่า high เพื่อให้ ปล่อย mutex เร็วที่สุด, ลดอาการ priority inversion ที่อาจถูกแทรกกลางโดย Medium-priority ที่ไม่เกี่ยว
3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
- Task ที่ priority สูง มีโอกาส ได้คิวก่อน และ preempt งานอื่น ๆ ระหว่างรอ/ทำงานนอก CS
ถ้า low ถือ mutex นาน ๆ จะ บล็อกทั้งระบบ จนกว่า low จะปล่อย (PI จะช่วยเร่งให้ปล่อย)
แนวปฏิบัติ: ทำ critical section สั้น, หลีกเลี่ยง vTaskDelay()/I/O หนัก ๆ ใน CS, ไม่ทำ nested locks, กำหนด timeout กับ xSemaphoreTake() และออกแบบลอจิก fallback
