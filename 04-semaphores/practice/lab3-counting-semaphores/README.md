...
|-------|-----------|-----------|--------------|----------|---------------------|
| 1 (3R, 5P) | 3 | 5 | 92.4% | 680 ms | 93% |
| 2 (5R, 5P) | 5 | 5 | 99.1% | 160 ms | 74% |
| 3 (3R, 8P) | 3 | 8 | 84.7% | 1280 ms | 98% |

### คำถามสำหรับการทดลอง
1. เมื่อ Producers มากกว่า Resources จะเกิดอะไรขึ้น?
- มี การแข่งขันทรัพยากร (contention): ผู้ผลิตบางส่วนต้อง รอคิว ที่ xSemaphoreTake() นานขึ้น และอาจ timeout → failed acquisition ได้ ถี่ขึ้นในช่วงโหลดพุ่ง (จาก LoadGen) ทำให้ Success Rate ลดลง และ Avg Wait เพิ่มขึ้น ขณะที่ Resource Utilization ติดเพดาน ใกล้ 100%
2. Load Generator มีผลต่อ Success Rate อย่างไร?
- LoadGen สร้าง burst แย่งทรัพยากรในช่วงสั้น ๆ ทำให้:
คิวรอพุ่ง → Avg Wait เพิ่มขึ้นชั่วคราว
โอกาส timeout สูงขึ้น → Success Rate ลดลง ในหน้าต่างเวลานั้น
หลัง burst จบ ระบบกลับสู่สมดุลเร็วถ้า R เพียงพอ
3. Counting Semaphore จัดการ Resource Pool อย่างไร?
ทำหน้าที่เป็น “ประตู” ที่อนุญาตให้มี การถือทรัพยากรพร้อมกันเท่ากับค่าของ semaphore (เริ่มต้น = จำนวน resource จริง)
ขั้นตอน: ก่อนใช้ resource: xSemaphoreTake() ถ้าค่า > 0 → ผ่านและ ลด count ลง 1
เลือก/จอง resource ว่างจริงจากอาร์เรย์ resources[]
เสร็จงาน: คืน resource และ xSemaphoreGive() เพื่อ เพิ่ม count กลับ 1
ผลลัพธ์: ป้องกันการใช้เกินจำนวน, ทำ rate limiting/parallelism cap ได้ตรงไปตรงมา และทำให้ utilization ติดตามได้ชัด
