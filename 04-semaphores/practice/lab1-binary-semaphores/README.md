...
|-------|-------------|-----------------|--------------|----------------|------------|
| 1 (Normal) | 180 | | 176 | 75 | 12 | ระบบสมดุล ส่วนใหญ่รับทัน มี timeout บ้างเล็กน้อย |
| 2 (Multiple Give) | 260 | 185 | 76 | 10 | มีการ Give ซ้ำ ระหว่างที่ semaphore ยังเป็น “1” → ส่วนเกินถูก ทิ้ง ทำให้ Sent สูงแต่ Received ไม่เพิ่ม |
| 3 (Short Timeout) | 170 | 150 | 74 | 9 | xSemaphoreTake(..., 3000ms) ทำให้ timeout เกิดบ่อยขึ้นเมื่อเหตุการณ์間ห่าง |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
- Binary semaphore มีค่าได้แค่ 0/1
2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
- ใน ISR ต้องใช้ xSemaphoreGiveFromISR() เท่านั้น
ใช้คู่กับตัวแปร xHigherPriorityTaskWoken และอาจเรียก portYIELD_FROM_ISR() เพื่อสลับไปยัง task ที่ถูกปลุกทันที (ลด latency)
3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
- สรุป: Semaphore = “มีเหตุการณ์หรือยัง?”, Queue = “เหตุการณ์อะไร / ข้อมูลอะไร?”
